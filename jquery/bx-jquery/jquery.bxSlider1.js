...
(function($){
	...
	$.fn.bxSlider = function(options){		
		...
		var defaults = {
			...
			wrapperClass: 'slider',					// string - classname attached to the slider wraper
				...
				if(!isWorking){
					...
					if(currentSlide < 0){
						...
						if(options.hideControlOnEnd){
							$outerWrapper.children('.bx-prev1').hide();
						}
					}
					checkEndControls();
					// next slide callback
		...
		this.destroyShow = function(){			
			...
			$outerWrapper.children('.bx-next1, .bx-prev1, .bx-pager, .bx-auto').remove();
		...
		function showControls(nextType, nextContent, prevType, prevContent){
			...
			var $nextHtml = $('<a href="" class="bx-next1"></a>');
			var $prevHtml = $('<a href="" class="bx-prev1"></a>');
		...
		function checkEndControls(){
			if(!options.infiniteLoop && options.hideControlOnEnd){
				...
				if(currentSlide == firstSlide){
					...
					$outerWrapper.children('.bx-prev1').show();
				}
				// check next
        ...
        if(currentSlide >= lastPossibleSlide){
					$outerWrapper.children('.bx-next1').hide();
				}else{
					$outerWrapper.children('.bx-next1').show();
				}
			}
		}
